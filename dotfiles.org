...
* notes for setting up "pre/initial" base VM image

~systemsetup -setremotelogin on~ turns on ssh
https://discussions.apple.com/message/9191821

ssh from host to guest
https://discussions.apple.com/message/9191821

for networking, use nat, forward port 3333 -> 22, can ssh via
~ssh -p 3333 testuser@localhost~
VBoxManage modifyvm "Mavericks" --natpf1 "guestssh,tcp,,3333,,22"
* How to prepare a VM for testing
...
to Mavericks desktop. Export it so you can re-import it if something
goes wrong.
...
** Re-import the VM
At this point, you don't actually need to do this, but if you want to
re-create from the previous point, here's how:

First, run the command with --dry-run
#+BEGIN_SRC
VBoxManage import ~/Documents/mavericks-base.ova --dry-run
#+END_SRC
...
#+END_SRC
** Export the VM part two
going to try to do one thing in the vm:
~systemsetup -setremotelogin on~
Then re-export.

also set the keyboard, but not interested in that.
** Setup the VM for testing purposes
All I really want to do is enable ssh on the VM so I can
easily access it: ~systemsetup -setremotelogin on~ from the command
line on the guest will do it.
...
ssh -p 3333 testuser@localhost
#+END_SRC
** add non-strict host checking
   ON my host, I have a problem with the above ssh line because keys
   have changed. In order to get around this, an alternative version
   looks like:
 #+BEGIN_SRC
