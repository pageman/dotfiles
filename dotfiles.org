...
** TODO refactor rests to "chef-env" script that runs other scripts in reset environment
    e.g. something like
...
** TODO virtual box
** TODO vagrant
** TODO upgrade to newest versions of pivotal sprout recipes
...
** TODO create a rake task for tangling dotfiles.org
   also going to need tangling for other files when this file is split
   would like to have this publishable
...
** TODO remove site-cookbooks/dmg
   IIRC this is was added in an attempt to understand things
   better/remove some frustration, but I think this really should just
   be installed through librarian-chef. The only reason I could see is
   because bootstrap might need to run before librarian-chef is run,
   but I can't see why that would be the case.
** TODO use an integration-test branch specific to integ. testing
   that way can keep master cleaner for things I am trying to do.
** TODO add tests for:
*** Firefox
